  ...
  return (
    <aside className="w-72 bg-brand-secondary shadow-2xl flex-col hidden md:flex">
      ...
      <nav className="flex-1 px-6 py-8 space-y-3">
        ...
        {user && user.role === 'user' && (
          <NavLink to="/file-a-case" className={getNavLinkClass}>
              <span className="material-icons mr-4">add_box</span>
              <span className="font-medium text-lg">File a New Case</span>
          </NavLink>
        )}
        {user && (
          <NavLink to="/history" className={getNavLinkClass}>
              <span className="material-icons mr-4">history</span>
              <span className="font-medium text-lg">Case History</span>
          </NavLink>
        )}

        {/* --- THIS IS THE FIX --- */}
        {/* Link 4: Find Arbitrator (Now ALWAYS Visible) */}
        <NavLink to="/find-arbitrator" className={getNavLinkClass}>
